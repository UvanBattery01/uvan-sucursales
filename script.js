...
function mostrarPopup() {
  if (!popup || !mensaje || mensajes.length === 0) return;

  let i;
  do {
    i = Math.floor(Math.random() * mensajes.length);
  } while (i === ultimoIndice && mensajes.length > 1);
...
function calcularEstadoSucursal(ahora, config) {
  ...
  if (config.closedOnSunday && dia === 0) {
    return estadoHTML("closed", "🔴 Cerrado", "Los domingos no abre");
  }
  ...
  if (ahora >= abre && ahora < cierra) {
    ...
    if (faltan <= 60) {
      return estadoHTML("closing-soon", "🟠 Cierra pronto", `Cierran en ${hm(faltan)}`);
    }

    return estadoHTML("open", "🟢 Abierto ahora", `Cierran a las ${formato(h.cierra.h, h.cierra.m)}`);
  }
  ...
  if (ahora >= cierra) {
    ...
    if (nd >= 1 && nd <= 5) {
      proxima.setHours(horarios.semana.abre.h, horarios.semana.abre.m, 0, 0);
    } else if (nd === 6) {
      proxima.setHours(horarios.sabado.abre.h, horarios.sabado.abre.m, 0, 0);
    } else {
      proxima.setHours(horarios.domingo.abre.h, horarios.domingo.abre.m, 0, 0);
    }
  }
  ...
  if (faltan <= 60) {
    return estadoHTML("opening-soon", "🟠 Abre pronto", `Abren en ${hm(faltan)}`);
  }

  return estadoHTML("closed", "🔴 Cerrado", `Abren ${proxima.toLocaleTimeString("es-MX", { hour: "numeric", minute: "2-digit" })}`);
}
...
function actualizarEstados() {
  ...
  const configs = {
    ...
    "estado-losreyes": { closedOnSunday: false },
    "estado-ixtapaluca": { closedOnSunday: false },
    "estado-chimalhuacan": { closedOnSunday: false },
    "estado-ermita": { closedOnSunday: false }
  };

  sucursales.forEach((id) => {
    const html = calcularEstadoSucursal(ahora, configs[id] || { closedOnSunday: false });
    aplicar(id, html);
  });
}
...
if (typeof media.addEventListener === "function") {
  ...
  media.addListener(actualizarTema);
}
...
function buscarSucursalMasCercana() {
  ...
  navigator.geolocation.getCurrentPosition(
    (pos) => {
      ...
      boton.onclick = () => window.open(mejor.maps, "_blank", "noopener,noreferrer");
    },
    () => {
